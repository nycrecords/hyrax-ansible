...
(echo -n "$(date --rfc-3339=seconds) - delete older monthly backups - " && find "{{ hyrax_backups_directory }}/monthly" -name='*.tar.gz' -type=f -mindepth=1 -maxdepth=1 -mtime=+365 -delete -print) >> /var/log/hyrax/backup.log 2>&1
