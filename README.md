...
/usr/local/bin/bundle exec rails hyrax:default_collection_types:create hyrax:default_admin_set:create hyrax:workflow:load
```
...
|Variable|Notes|
|---|---|
|`ansistrano_deploy_to` | Destination for app deployment, defaults to: "/var/www/hyrax" |
|`ansistrano_keep_releases` | Number of old releases to keep, defaults to: 5 |
|`ansistrano_shared_paths` | Array of shared paths that are symlinked to each release |
|`ansistrano_shared_files` | Array of shared files that are symlinked to each relaese |
|`ansistrano_deploy_via` | Ansistrano tool for depoyment, can by rsync or git, defaults to git |
|`ansistrano_git_repo` | Repo of the application to deploy, defaults to: https://github.com/samvera/hyku.git |
|`ansistrano_git_branch` | What version of the repository to check out. This can be the full 40-character SHA-1 hash, the literal string HEAD, a branch name, or a tag name. Defaults to: master |
